...
def run_hpc_simulation(
    ...
    fast_mode = True:
        - uses DC power flow (pp.rundcpp)
    fast_mode = False:
        - uses AC power flow (pp.runpp)

    grid_backend:
        - Synthetic HPC grid: original custom 110/20/0.4 kV HPC model
        - SimBench German benchmark grid: public German benchmark network with added HPC loads
    """
...
def calculate_energy(results_df: pd.DataFrame):
    df = results_df.copy()
