"""
Workload Profile Builder with Multi-GPU and Multi-Trace Support

Enhanced version that:
- Supports multi-GPU traces
- Handles distributed workload aggregation
- Validates power trace data quality
- Supports custom workload patterns
"""
...
def _average_optional_column(dfs: list[pd.DataFrame], column: str) -> Optional[pd.Series]:
...
def validate_trace_quality(profile: pd.DataFrame) -> dict:
    """Validates the quality of a power trace and returns statistics."""
    stats = {
        "samples": len(profile),
        "duration_hours": profile["elapsed_hours"].iloc[-1] if len(profile) > 0 else 0,
        "avg_power_w": profile["total_gpu_power_w"].mean(),
        "peak_power_w": profile["total_gpu_power_w"].max(),
        "min_power_w": profile["total_gpu_power_w"].min(),
        "power_std": profile["total_gpu_power_w"].std(),
        "zero_power_samples": (profile["total_gpu_power_w"] == 0).sum(),
        "has_gpu_util": "gpu_util_percent" in profile.columns,
        "has_cpu_util": "cpu_util_percent" in profile.columns,
    }
    return stats
...
def build_measured_profile(folder_path: str | Path, workload_label: str = "Training") -> pd.DataFrame:
    ...
    if avg_cpu_util is not None:
        ...
        profile["cpu_util_percent"] = 50.0  # Reasonable default when not measured
    ...
    # Add quality metrics
    profile["trace_quality"] = "validated"
...
def build_selected_workload_profile(
    ...
    - Simultaneous Training + Inference: training trace plus a repeated inference
      pattern, aligned to the training timeline.
    """
